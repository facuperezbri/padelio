...
export function NavigationProvider({ children }: { children: ReactNode }) {
  const [confirmHandler, setConfirmHandler] =
    useState<NavigationConfirmHandler | null>(null);

  const registerConfirmHandler = useCallback(
    (handler: NavigationConfirmHandler | null) => {
  ...
  return (
    <NavigationContext.Provider
      value={{ registerConfirmHandler, handleNavigation }}
    >
      {children}
    </NavigationContext.Provider>
  );
