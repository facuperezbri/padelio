...
-- First, drop update_match_elos function since it depends on calculate_new_elo
DROP FUNCTION IF EXISTS update_match_elos(UUID);

-- Drop ALL possible versions of calculate_new_elo function using CASCADE
-- This will remove all overloaded versions
DO $$
DECLARE
  r RECORD;
BEGIN
  -- Find and drop all versions of calculate_new_elo
  FOR r IN 
    SELECT oid::regprocedure::text as func_signature
    FROM pg_proc
    WHERE proname = 'calculate_new_elo'
  LOOP
    EXECUTE 'DROP FUNCTION IF EXISTS ' || r.func_signature || ' CASCADE';
  END LOOP;
END $$;

-- Create the correct version (with total_matches_played INT parameter)
...
-- Recreate update_match_elos function with the correct signature
