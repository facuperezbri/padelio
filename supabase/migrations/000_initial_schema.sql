...
CREATE INDEX IF NOT EXISTS idx_profiles_elo ON profiles(elo_score DESC);
CREATE INDEX IF NOT EXISTS idx_profiles_username ON profiles(username);

CREATE INDEX IF NOT EXISTS idx_players_profile ON players(profile_id);
CREATE INDEX IF NOT EXISTS idx_players_creator ON players(created_by_user_id);
CREATE INDEX IF NOT EXISTS idx_players_ghost ON players(is_ghost);
CREATE INDEX IF NOT EXISTS idx_players_elo ON players(elo_score DESC);
...
CREATE INDEX IF NOT EXISTS idx_matches_player3 ON matches(player_3_id);
CREATE INDEX IF NOT EXISTS idx_matches_player4 ON matches(player_4_id);
...
-- Note: handle_new_user and handle_new_profile triggers will be created in migration 006
-- to ensure they use the latest version with proper conflict handling
...
DROP POLICY IF EXISTS "Users can create ghost players" ON players;
DROP POLICY IF EXISTS "Users can update own or created players" ON players;
DROP POLICY IF EXISTS "Users can delete own ghost players" ON players;
...
DROP POLICY IF EXISTS "Match creator can create invitations" ON match_invitations;
DROP POLICY IF EXISTS "Users can update their invitations" ON match_invitations;
